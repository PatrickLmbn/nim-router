...
import {
  Sun, Moon, RefreshCw, Key, Server, Sliders, Terminal, Shield, Activity, RotateCcw, CheckCircle2, AlertCircle, GitMerge, Lock, BarChart3, X, Copy
} from 'lucide-react';
import nimCubeLogo from '../icons/nim-cube.svg';
import { BentoGrid } from './components/BentoGrid';
import { LoginPage } from './components/LoginPage';
import { authFetch } from './api';
import { duplicateCombo } from './duplicateCombo.mjs';
import {
  ModelSelectorModal,
  KeysManagerModal,
  SettingsModal,
...
export default function App() {
  ...
  const [duplicateDialog, setDuplicateDialog] = useState({ open: false, sourceName: '', defaultName: '' });
  ...
  const handleDuplicateCombo = async (name) => {
    if (duplicateInFlight.current) return;
    const combos = await (await authFetch('/api/combos')).json();
    const source = combos.combos?.find(c => c.name.toLowerCase() === name.toLowerCase());
    const defaultName = source ? `${source.name}-copy` : `${name}-copy`;
    setDuplicateDialog({ open: true, sourceName: name, defaultName });
  };

  const handleDuplicateConfirm = async (customName) => {
    if (duplicateInFlight.current) return;
    duplicateInFlight.current = true;
    const { sourceName } = duplicateDialog;
    setDuplicatingCombo(sourceName);
    setDuplicateDialog({ open: false, sourceName: '', defaultName: '' });
    try {
      const copy = await duplicateCombo(sourceName, authFetch, customName);
      showToast(`Combo "${copy.name}" created`, 'success');
      await fetchStats();
    } catch (error) {
      showToast(error.message || 'Failed to duplicate combo', 'error');
    } finally {
    ...
    <div className="min-h-screen lg:h-screen lg:max-h-screen lg:overflow-hidden flex flex-col justify-between py-2 px-2 sm:px-6 bg-[#eef2f7] dark:bg-[#0f1117] text-slate-800 dark:text-slate-100 transition-colors duration-300">
      ...
      <main className="flex-1 w-full flex flex-col lg:justify-center items-center px-1 sm:px-4 lg:px-6 py-1.5 sm:py-2 lg:min-h-0">
        <BentoGrid
          ...
          duplicatingCombo={duplicatingCombo}
      ...
      {duplicateDialog.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => setDuplicateDialog({ open: false, sourceName: '', defaultName: '' })}>
          <div className="w-full max-w-md mx-4 bg-white dark:bg-[#181d28] rounded-2xl border border-black/5 dark:border-white/10 shadow-2xl p-5 animate-slide-up" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Duplicate Combo</h3>
              <button onClick={() => setDuplicateDialog({ open: false, sourceName: '', defaultName: '' })} className="p-1 rounded-lg hover:bg-black/5 dark:hover:bg-white/10 text-slate-500" title="Cancel">
                <X className="w-4 h-4" />
              </button>
            </div>
            <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">Enter a name for the duplicated combo:</p>
            <input
              type="text"
              defaultValue={duplicateDialog.defaultName}
              ref={duplicateInputRef}
              onKeyDown={(e) => { if (e.key === 'Enter') handleDuplicateConfirm(duplicateInputRef.current?.value.trim() || duplicateDialog.defaultName); if (e.key === 'Escape') setDuplicateDialog({ open: false, sourceName: '', defaultName: '' }); }}
              className="w-full px-3 py-2 rounded-xl bg-slate-100 dark:bg-black/40 border border-black/10 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-[#00d2ff] focus:ring-1 focus:ring-[#00d2ff] text-sm font-mono"
              placeholder="Combo name"
            />
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setDuplicateDialog({ open: false, sourceName: '', defaultName: '' })} className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/10 text-sm font-semibold transition neu-button">Cancel</button>
              <button onClick={() => handleDuplicateConfirm(duplicateInputRef.current?.value.trim() || duplicateDialog.defaultName)} disabled={duplicateInFlight.current} className="px-4 py-2 rounded-xl bg-[#00d2ff] hover:bg-[#3a7bd5] text-white text-sm font-semibold transition neu-button disabled:opacity-50 flex items-center gap-1.5">
                {duplicateInFlight.current ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Copy className="w-3.5 h-3.5" />}
                <span>{duplicateInFlight.current ? 'Duplicating...' : 'Duplicate'}</span>
              </button>
            </div>
          </div>
        </div>
